...
namespace Foam
{
    defineTypeNameAndDebug(mlpCurvatureNormal, 0);
    addToRunTimeSelectionTable(curvatureModel,mlpCurvatureNormal, components);
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::mlpCurvatureNormal::mlpCurvatureNormal
(
    ...
    stencil_(mesh_,ijkMesh_,2)
{
  ...
  if (!isUniform)
    FatalErrorInFunction
      << "mlpCurvatureNormal requires uniform grids!"
      << abort(FatalError);
  ...
  Info<<"mlpCurvatureNormal: grid spacing, dl_="<<dl_<<endl;

  nMax_=max(iMax_,jMax_);
  nMax_=max(nMax_,kMax_);
  
  ijkMesh_.markBoundaryCells(boundaryCells_,nMax_);
  Info<<"mlpCurvatureNormal: done setting up ijkMesh..."<<endl;
  ...
  Info<<"mlpCurvatureNormal:stencil size="<<NInput_<<endl;
  if (is2D_)
    {
  ...
  if (is2D_)
    {      
      ...
      Info<<"mlpCurvatureNormal currently supports only the x-z coordinate system in 2D."<<endl;
...
void Foam::mlpCurvatureNormal::correctContactAngle
(
...
void Foam::mlpCurvatureNormal::calculateK()
{
  Info<<"mlpCurvatureNormal: calculating the curvature..."<<endl;
  const fvMesh& mesh = alpha1_.mesh();
			  ...
			  for (int n=-nMax;n<nMax+1;n++)
			    {
			      ...
			      indices_[iA]=stencil_.a2(nF*n,tF*t);
			      iA+=1;
			    }
			}
		    }
