...
namespace Foam
{
    defineTypeNameAndDebug(mlpCurvatureRDF, 0);
    addToRunTimeSelectionTable(curvatureModel,mlpCurvatureRDF, components);
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::mlpCurvatureRDF::mlpCurvatureRDF
(
    ...
    stencil_(mesh_,ijkMesh_,2)
{
  ...
  if (!isUniform)
    FatalErrorInFunction
      << "mlpCurvatureRDF requires uniform grids!"
      << abort(FatalError);
  ...
  Info<<"mlpCurvatureRDF: grid spacing, dl_="<<dl_<<endl;

  nMax_=max(iMax_,jMax_);
  nMax_=max(nMax_,kMax_);
  
  ijkMesh_.markBoundaryCells(boundaryCells_,nMax_);
  Info<<"mlpCurvatureRDF: done setting up ijkMesh..."<<endl;
  ...
  Info<<"mlpCurvatureRDF:stencil size="<<NInput_<<endl;
  if (is2D_)
    {
...
void Foam::mlpCurvatureRDF::correctContactAngle
(
...
void Foam::mlpCurvatureRDF::calculateK()
{
  Info<<"mlpCurvatureRDF: calculating the curvature..."<<endl;
  const fvMesh& mesh = alpha1_.mesh();
