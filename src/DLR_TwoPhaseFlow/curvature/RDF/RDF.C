    ...
    RDF_(alpha1.mesh())
{
    ...
    Info<<"curvature from trace?: "<<curvFromTr_<<endl;
}
...
Foam::labelList Foam::RDF::getNeibourProcs(const boolList& nextToInterface)
{
    ...
    labelList myProc;
    myProc.append(Pstream::myProcNo());
...
void Foam::RDF::calculateK()
{
    ...
    constructRDFOctree(RDF_,FaceCentreField_,FaceNormalField_);
...
void Foam::RDF::constructRDFOctree
(
    ...
    forAll(reconDistFunc.boundaryField(), patchI)
    {
        if (reconDistFunc.boundaryField().types()[patchI] == "calculated")
        {
            ...
            forAll(pRDF, i)
            {
                ...
                {
                    const point& p = mesh.C().boundaryField()[patchI][i];
                    pointIndexHit pHit =  surfaceTree.findNearest (p, GREAT);
                    const label idx = pHit.index();
                    if (idx == -1)
                    {
